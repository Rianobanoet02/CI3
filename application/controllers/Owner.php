...
class Owner extends CI_Controller
{
    ...
    {
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        $data['bahan'] = $this->admin_model->get_slaporan();
    ...
    public function print_bahan(){
        $datauser = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        if($datauser){$data['title'] = 'Pencarian';
        $data['bahan'] = $this->admin_model->get_slaporan();
        $data['sum'] = $this->admin_model->get_sum();
        $tgl = $this->input->post('tgl_pembelian');
        $data['laporan_print'] = $this->admin_model->laporan_print($tgl);
        $this->load->view( 'backend/owner/laporan_print', $data);
        //$this->load->view('templates/master');
		}else{
			echo "data belum berhasil";
		}
    ...
    public function pencarian(){
        $datauser = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        if($datauser){
			echo "data belum belum ada";
		}else{
		$data['title'] = 'Pencarian';
        $this->template->load('backend/owner/pencarian', $data);
	}
    }
